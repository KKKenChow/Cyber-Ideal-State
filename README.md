...
当使用聊天记录等数据源进行蒸馏时，系统会调用 LLM 对数据进行分析。以下是各分析器的 Token 消耗估算：

| 分析器 | LLM 调用次数 | 输入 Tokens | 输出 Tokens | 合计 |
|--------|-------------|-------------|-------------|------|
| PersonaAnalyzer（人格分析） | 1 | ~2,200 | ~300 | **~2,500** |
| MemoryAnalyzer（记忆提取） | 最多 5 | ~6,750 | ~1,000 | **~7,750** |
| RelationshipAnalyzer（关系分析） | 1 | ~1,120 | ~200 | **~1,320** |
| **总计** | **最多 7 次** | | | **~11,500 - 12,000** |

> **为什么这么少？** 代码中对每条消息做了截断处理（100-200 字）并采用采样机制（最多取 20-50 条），避免一次性灌入全部聊天记录。

**实际影响因素：**

| 场景 | Token 消耗 | 说明 |
|------|-----------|------|
| 聊天记录量少（< 20 条） | **~2,500** | MemoryAnalyzer 和 RelationshipAnalyzer 不触发 LLM 调用 |
| 聊天记录量大（> 1000 条） | **~12,000** 封顶 | 采样机制保证上限 |
| 仅手动描述（无聊天数据） | **0** | 不调用任何 LLM，零 Token 消耗 |
| 没有 OpenClaw | **0** | LLM 客户端不可用时，仅做基础统计 |

**费用估算（按 OpenAI 定价）：**
- GPT-4o 级别：约 **$0.03 - 0.06** / 次
- GPT-4o-mini 级别：约 **$0.003 - 0.006** / 次

#### 纯手动模式：不用蒸馏也能完整塑造人物

如果你没有聊天记录导出文件，或者不想消耗 Token，**纯手动描述也能完成人物信息的所有塑造**。两种模式的对比：

| 能力维度 | 蒸馏模式（聊天记录 → LLM 分析） | 纯手动模式（UI 填写） |
|----------|-------------------------------|---------------------|
| **人格标签（tags）** | ✅ 自动提取 MBTI、性格标签 | ✅ 在「描述」和「手动描述」中自由填写 |
| **说话风格（speaking_style）** | ✅ 自动分析消息长度、活跃时段、emoji 使用率 | ✅ 在「手动描述」中描述"他/她怎么说话" |
| **情感模式（emotional_pattern）** | ✅ 自动分析情感表达频率 | ✅ 在「手动描述」中描述"他/她的情绪特点" |
| **共同经历（shared_experiences）** | ✅ 自动从聊天记录提取重要事件 | ✅ 在「手动描述」中写"我们一起经历过的事" |
| **内部梗（inside_jokes）** | ✅ 自动识别专属笑点 | ✅ 在「手动描述」中写"我们之间的梗" |
| **关系洞察（relationship_insights）** | ✅ 自动分析互动模式 | ✅ 在「手动描述」中写"我们的关系特点" |
| **时间线（timeline）** | ✅ 自动从消息时间戳生成 | ❌ 不支持（需聊天数据时间戳） |
| **互动频率统计** | ✅ 自动计算消息数、发起率、回复时长 | ❌ 不支持（需聊天数据） |
| **Token 消耗** | ~2,500 - 12,000 | **0** |
| **数据隐私** | 需上传聊天记录到 LLM API | **完全本地，不调用任何 LLM** |
| **准确度** | 基于真实数据，客观 | 基于主观认知，可能更深刻 |

**结论：** 纯手动模式可以覆盖人格塑造的**所有主观维度**（性格、说话风格、情感、记忆、关系），唯一缺失的是需要原始数据才能计算的**客观统计指标**（时间线、互动频率、回复时长）。对于大多数使用场景，纯手动描述已经足够让 Agent "像那个人"——毕竟你最了解你身边的人。

**纯手动模式使用方法：**
1. 创建角色时，数据来源选择「手动描述」
2. 在「手动描述」文本框中尽量详细地描述，建议包含：
   - **性格特征**：他/她是什么样的人（MBTI、标签等）
   - **说话风格**：他/她平时怎么说话（口头禅、语气、用词习惯）
   - **情感模式**：他/她的情绪特点（感性/理性、表达方式）
   - **共同经历**：你们一起经历过的重要事件
   - **关系特点**：你们的关系模式和互动方式
   - **内部梗**：只有你们懂的笑点
3. 点击「创建角色」— 系统会跳过 LLM 分析，将你的描述直接写入 Agent 的 SOUL.md
